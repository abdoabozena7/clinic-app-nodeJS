...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  -- `read` is a reserved word in SQL; wrap it in backticks to avoid syntax errors
...
-- Additional doctor user for Cardiology with a different price
INSERT IGNORE INTO users (name, email, phone, password, role) VALUES
  ('Dr. Daisy', 'daisy@example.com', '01000000005', 'doctor123', 'doctor');

-- Insert doctor profiles referencing the corresponding users
INSERT IGNORE INTO doctors (userId, specialty, bio, imageUrl, location, price) VALUES
  (2, 'Cardiology', 'Experienced cardiologist.', '', 'Cairo', 200.00),
  (3, 'Pediatrics', 'Expert pediatrician.', '', 'Giza', 150.00),
  (4, 'Dermatology', 'Skin specialist.', '', 'Cairo', 250.00),
  -- Additional doctors for same specialty with different prices
  (6, 'Cardiology', 'Senior cardiologist with 20+ years of experience.', '', 'Alexandria', 300.00);

-- Insert schedules for doctors
-- Doctor 1: Dr. Alice works Monday–Friday, 9 AM–5 PM
INSERT IGNORE INTO schedules (doctorId, dayOfWeek, startTime, endTime) VALUES
...
INSERT IGNORE INTO appointments (doctorId, userId, startTime, endTime, reason, status) VALUES
